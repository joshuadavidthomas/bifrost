...
Java, JavaScript, TypeScript, C++, C#, Go, PHP, Python, Ruby, Rust, and Scala provide bounded receiver traversal. Unsupported source forms, including plain C receiver traversal, preserve an explicit `unsupported` row and capability diagnostic. See [Receiver Traversal](/code-query-tutorials/receiver-traversal/) for allocation, factory, ambiguity, reference-site, and call-input examples with exact output.
