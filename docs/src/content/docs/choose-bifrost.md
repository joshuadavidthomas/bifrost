...
| “What value can this receiver denote, or which exact member does it select?” | [`receiver_targets`, `points_to`, or `member_targets`](/code-query-tutorials/receiver-traversal/) | Returns a bounded `receiver_analysis` row with explicit precision, ambiguity, unsupported, and budget outcomes for supported languages. |
...
Bifrost is a good fit when the answer can be grounded in parsed source structure, indexed declarations, exact source references, resolved call edges, direct project-file imports, indexed type relationships, or bounded receiver provenance in a supported language. It is not a path-sensitive control-flow, whole-program points-to, general alias, taint, or whole-program data-flow engine. Read the [capability matrix](/capabilities/) for language-specific boundaries before relying on a zero-result or completeness claim.
