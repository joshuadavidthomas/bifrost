...
Stable outcomes are `precise`, `ambiguous`, `unknown`, `unsupported`, and `exceeded_budget`. Ordinary bounded ambiguity retains every candidate and does not set top-level `truncated`. Candidate-cap truncation and `exceeded_budget` do set `truncated` and emit an aggregated limit diagnostic. Supported adapters return structured candidates where their neutral facts prove them and preserve dynamic or unmodeled boundaries as `ambiguous`, `unknown`, or `unsupported`; plain C remains explicitly unsupported for receiver traversal. Receiver-analysis rows are terminal except for `file_of`.
