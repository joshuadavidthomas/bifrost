...
use brokk_bifrost::semantic_packs::summary_foundry::taint_policy::build_require_model_java_taint_policy;
use brokk_bifrost::summary_foundry_demand::{
    SweepConfig, completion_bucket, load_codeql_endpoints, run_realistic_sweep, run_sweep,
    select_java_slice, write_policy, write_report,
};
use brokk_bifrost::{AnalyzerConfig, FilesystemProject, Project, WorkspaceAnalyzer};

/// The checked-in curated CodeQL injection model slice the realistic sweep
/// translates its endpoints from.
fn injection_models_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("semantic-packs/summary-foundry/codeql-java-injection-models")
}
...
/// The whole milestone-4.6 chain, hermetically: the checked-in CodeQL injection
/// rows translate into endpoints, those build a require-model Java policy, and
/// that policy -- run through the production evaluator over a tiny workspace that
/// flows `request.getParameter(..)` into `stmt.executeQuery(..)` -- matches the
/// real call sites and analyzes the flow.
///
/// A clean zero-finding, zero-blocker conclusion would mean the selectors matched
/// nothing, which is exactly the milestone-4.5 degeneracy this milestone fixes.
/// So the proof is that the run produces either a finding or an attributed
/// blocker: both require the source and the sink selectors to have matched.
#[test]
fn the_built_policy_matches_real_java_and_runs_through_the_evaluator() {
    let endpoints = load_codeql_endpoints(&injection_models_dir()).expect("endpoints load");
    assert!(
        endpoints.len() >= 20,
        "the curated injection slice should carry a couple dozen endpoints, got {}",
        endpoints.len()
    );
    let (policy, shape) = build_require_model_java_taint_policy(&endpoints);
    assert!(shape.sources >= 1 && shape.sinks >= 1, "shape {shape:?}");

    let directory = tempfile::tempdir().expect("workspace root");
    let root = directory.path();
    let source = "import javax.servlet.http.HttpServletRequest;\n\
         import java.sql.Statement;\n\
         public class App {\n\
         \x20 void handle(HttpServletRequest request, Statement stmt) throws Exception {\n\
         \x20   String name = request.getParameter(\"name\");\n\
         \x20   String query = \"SELECT * FROM users WHERE name = '\" + name + \"'\";\n\
         \x20   stmt.executeQuery(query);\n\
         \x20 }\n\
         }\n";
    std::fs::create_dir_all(root.join("src")).expect("src dir");
    std::fs::write(root.join("src/App.java"), source).expect("App.java");
    ...
    let inputs = [PolicyEvaluationInput::embedded(
        PolicySourceIdentity::new("bifrost:summary-foundry/realistic-policy-test"),
        &policy,
    )];
    let options = PolicyEvaluationOptions::new(
        PolicyEvaluationDate::from_ymd(2026, 1, 1).expect("a fixed date"),
    ...
    let run = outcome
        ...
        .iter()
        .find(|run| run.analysis_type() == PolicyAnalysisType::Taint)
        .expect("the built policy produced a taint run");
    ...
    assert!(
        findings > 0 || !blockers.is_empty(),
        "the source->sink flow must produce a finding or an attributed blocker; \
         completion={:?}, findings={findings}, blockers={blockers:#?}",
        run.completion()
    );
}
...
/// The milestone-4.6 realistic corpus sweep, run once as a measurement. It
/// translates the checked-in CodeQL injection endpoints, builds the require-model
/// Java policy, selects the slice by whether a repo imports the endpoint
/// libraries, runs the policy over each selected repo, and writes both the
/// deterministic report and the built policy so the run is reproducible.
///
/// Run it with, for example:
///     BIFROST_DEMAND_SWEEP_CORPUS=/home/jonathan/Projects/brokkbench/clones \
///     BIFROST_DEMAND_SWEEP_MAX_REPOS=12 \
///     cargo test --features release-tooling --test suite_bench_policy -- \
///       --ignored summary_foundry_demand::runs_the_realistic_corpus_demand_sweep
#[test]
#[ignore = "measurement over the real corpus; run once to produce the committed artifact"]
fn runs_the_realistic_corpus_demand_sweep() {
    let Ok(corpus_root) = std::env::var("BIFROST_DEMAND_SWEEP_CORPUS") else {
        panic!("set BIFROST_DEMAND_SWEEP_CORPUS to the corpus root");
    };
    let max_repos = std::env::var("BIFROST_DEMAND_SWEEP_MAX_REPOS")
    ...
    let report_out = std::env::var("BIFROST_DEMAND_SWEEP_OUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            manifest.join("semantic-packs/summary-foundry/demand-sweep-java-realistic.json")
        });
    let policy_out = std::env::var("BIFROST_DEMAND_SWEEP_POLICY_OUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            manifest.join(
                "semantic-packs/summary-foundry/attacker-controlled-injection.require-model.rqlp",
            )
        });

    let endpoints = load_codeql_endpoints(&injection_models_dir()).expect("endpoints load");
    eprintln!("translated {} taint endpoints", endpoints.len());

    let mut config = SweepConfig::new(corpus_root, max_repos);
    config.per_repo_timeout = Duration::from_secs(240);

    let sweep = run_realistic_sweep(&config, &endpoints).expect("the realistic sweep runs");
    let report = &sweep.report;
    eprintln!(
        "policy: {} source selectors, {} sink selectors; import libraries: {:?}",
        sweep.policy_shape.sources, sweep.policy_shape.sinks, sweep.import_packages
    );
    eprintln!("selected {} java repos for the slice", report.repo_count);
    for outcome in &report.repo_outcomes {
        eprintln!(
            "  repo {} ({} java files) -> {:?}",
            outcome.repo, outcome.java_files, outcome.outcome
        );
    }
    eprintln!(
        "baseline clean-conclusion: {}/{} runs ({} permille); blocked verdicts: {}; \
         engine-blocked verdict share: {} permille; summary-closable blockers: {}, \
         engine-blocked blockers: {}",
        report.clean_conclusion_runs,
        report.evaluated_runs,
        report.clean_conclusion_permille,
        report.verdict_split.blocked_verdicts,
        report.verdict_split.engine_blocked_verdict_permille,
        report.blocker_bucket_totals.summary_closable,
        report.blocker_bucket_totals.engine_blocked,
    );
    for blocker in &report.ranked_blockers {
        eprintln!(
            "  blocker {} [{}] callee={:?} blocks {} verdicts",
    ...
    write_report(report, &report_out).expect("the report writes");
    write_policy(&sweep.policy, &policy_out).expect("the policy writes");
    eprintln!(
        "wrote {} and {}",
        report_out.display(),
        policy_out.display()
    ...
    // Every repo is accounted for in exactly one outcome bucket (no silent drop).
    let tallies = &report.outcomes;
    let accounted = tallies.complete
        + tallies.proven_by_summary
        + tallies.proven_subset
