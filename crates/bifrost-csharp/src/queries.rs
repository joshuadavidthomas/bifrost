...
//! folds [`CSHARP_QUERY_ASSETS`] into the per-language store epoch exactly as it
//! folds its own `resources/treesitter/<lang>/` files: the entry paths keep the
//! historical `treesitter/c_sharp/` prefix so the epoch's per-language filter is
//! one rule rather than two.

/// Directory the query assets live in, relative to this crate's root.
///
/// Reported by `CSharpAdapter::query_directory()`. The relocation from the
/// analysis crate is why the C# store-epoch salt bumped: the salted content now
/// comes from this crate's `resources/`, not analysis's.
pub const CSHARP_QUERY_DIRECTORY: &str = "resources/treesitter/c_sharp";

/// Compile-time embedded `.scm` query files as `(relative_path, contents)`.
///
/// C# ships no `identifiers.scm`; type identifiers come from the declaration
/// walk in [`crate::declarations`], not from a query.
pub const CSHARP_QUERY_ASSETS: &[(&str, &str)] = &[
    (
