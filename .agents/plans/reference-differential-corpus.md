...
- [ ] (2026-07-14) Filed #766 only after a consumer-only authoritative regression proved that forward lookup accepts tree-sitter's exact missing-`::` recovery for a macro-decorated function return type while inverse routing drops its `namespace_identifier`. The issue is assigned only to `jbellis`. Synchronous Oldskool implementation adds one shared structural predicate restricted to the direct function-definition qualified declarator, missing `::`, and pointer/reference-wrapped function-declarator shape; targeted and whole-workspace inverse walkers use it without broadly admitting namespace identifiers. The 63 C++ usage, 17 inverted graph, and 56 C++ get-definition tests pass, as does affected all-feature clippy. Root review accepts the implementation; exact RMerl production validation is still running under T9 contention before push and closure.
