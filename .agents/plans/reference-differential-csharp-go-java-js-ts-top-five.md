...
- [x] (2026-07-20) Published C# #984 at `0b05d428`: forward extension lookup now requires structured receiver-type evidence and matches the candidate's declared `this` type, so an external `System.Nullable` receiver cannot borrow ClosedXML's same-named local extension. Delegated validation passed all 543 definition tests, 32 C# graph tests, 103 targeted C# usage tests, formatting, and all-feature Clippy; a clean-head exact rerun remains before closure.
