...
2026-08-08 (implementation): `RunPolicyPreparation::Deadline` in `crates/bifrost-mcp/src/searchtools_service.rs` now boxes its `RunPolicyToolResult`. The new optional diff field grew `PolicyReportDocument` enough to trip clippy's large-enum-variant lint against the slim `Ready` handle; boxing the report-carrying variant is the indicated fix and changes no behavior.

Note on this revision: progress, surprises, decision log, and the retrospective were updated together with each milestone commit, per PLANS.md's living-document requirement; the decided semantics section is unchanged.
...
- [x] (2026-08-08) Milestone 4: five git-backed CLI tests in `tests/suite_bench_policy/bifrost_policy_cli.rs` (cross-format new-vs-persisting gating and the exit-code difference with and without the flag; a suppressed new finding not gating while the classification counts stay unchanged; the degraded base via an invalid committed suppressions document repaired in the working tree, asserting full gating, `degraded: true`, the `diff-base-unreliable` diagnostic, and no per-finding decisions; unresolvable revision and non-git root exiting 2; the rename limitation documented as `pure_rename_currently_reports_fixed_plus_new`); three new rows in the arg-validation exclusivity table (`--diff-base` twice, missing value, with `--list-policies`); docs sections in `static-analysis-policies.md` (test-enforced by the new `policy_docs::diff_gating_documentation_states_the_contract_and_limitations`) and `cli.md`; the CI page recipe landed with milestone 3.
- [x] (2026-08-08) Full validation: `cargo fmt` clean; `cargo test --test suite_bench_policy` 329 passed, 0 failed, 2 ignored; `cargo test -p brokk-bifrost-policy` 301 passed; `cargo test -p brokk-bifrost-mcp` 147 passed across targets; `cargo clippy --workspace --all-targets -- -D warnings` clean (one fix: `RunPolicyPreparation::Deadline` now boxes its report payload, because the report document grew past the large-enum-variant threshold). The pre-push gate remains for whoever pushes; this branch is not pushed.
...
2026-08-08: All four milestones are implemented and validated on this branch. A user can now run `bifrost --root . --policy-pack bifrost.code-smells --format sarif --output out.sarif --diff-base origin/master` and gate a pull request on exactly the findings it introduced, from the CLI, the MCP `run_policy` tool, and the reusable GitHub Action. The purpose stated at the top of this plan is met and demonstrated by end-to-end tests: one new finding in a repository with a pre-existing one fails with exactly that one finding gating; the unchanged worktree passes; a broken base degrades loudly; an unresolvable base fails outright.

What deviated from the plan's letter (each recorded in the decision log): the analysis crate exports one composed `export_revision` entry point instead of raising `RevisionImage`/`Snapshot` visibility; the diff types live in `finding.rs`/`report.rs` rather than `suppression.rs`; the base run keeps the default conventional suppression/scope loaders (identity-neutral) instead of a new disabled state; the report field, gate predicate, and degradation diagnostic landed with milestone 1 because the join is not representable without them.

Remains for follow-up (tracked as non-goals or new issues): warm base evaluation through the persisted blob-OID store; bulk baseline acceptance (#1881); rename-tracking and ordinal-stable identities; and one asymmetry discovered during implementation - an MCP head evaluation can hold active semantic models while the base evaluation runs without them, so a semantic-model-dependent policy could complete on the head and be non-exhaustive on the base, degrading the diff. That is safe (degradation, never silent misclassification) but worth an issue when semantic-model policies meet diff gating in practice.

Lesson learned: the retained-size budget machinery is the sharpest edge in this codebase. Every new report field must be charged in the builder's every retention decision, not only in the final document, or late retentions can push `finish()` over budget and fail the whole run. The `set_diff`/`diff_extra` pattern mirrors `set_execution` for this reason.
