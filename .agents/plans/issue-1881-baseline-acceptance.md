...
- [x] (2026-08-08) Full validation after merging master (#1873 policy-scan action changes, FIRD test-region filtering): `cargo nextest run --workspace --no-fail-fast` reached 9665 of 9676 passed with 11 `suite_mcp_cli::bifrost_benchmark_run` failures caused by the concurrent mid-run master merge mutating the spawned binary, not by this change — all 11 pass on a clean re-run; post-merge `suite_bench_policy` 346 passed and workspace clippy clean.
